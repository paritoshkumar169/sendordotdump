...
pub struct CreateLaunch<'info> {
    ...
    #[account(mut)]
    pub admin: Signer<'info>,            // must be global_state.admin
...
pub fn create_launch(ctx: Context<CreateLaunch>, base_price: u64, slope: u64) -> Result<()> {
    ...
    // Initialize LaunchMetadata fields
    ...
    // Store the PDA bump for LaunchMetadata (useful for signing CPIs)
    ...
    // Initialize BondingCurveState fields
    ...
    // Mint the initial fixed supply of tokens to the vault
    let seeds = &[b"launch", global.launch_count.to_le_bytes().as_ref(), &[launch.bump]];
    let signer = &[&seeds[..]];
    let total_supply: u64 = INITIAL_SUPPLY_BASE_UNITS;
    // CPI: mint the total supply to the vault (LaunchMetadata is mint authority)
    token::mint_to(
    ...
    // Revoke (set to None) the mint authority so no more tokens can be minted (fixed supply)
    token::set_authority(
        CpiContext::new_with_signer(
            ctx.accounts.token_program.to_account_info(),
            SetAuthority {
    ...
    // Increment global launch count for the next launch
